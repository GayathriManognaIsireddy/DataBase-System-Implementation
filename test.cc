...
void q6 () {
	...
	cout << "Complteted init of the DB files" << endl;
	Join J;
		// left _s
		// right _ps
		Pipe _s_ps (pipesz);
	...
	GroupBy G;
		...
		Function func;
			...
			grp_order.SetAttributes(1, grp_attr, grp_attrType);
			
	cout << "Setting the group attributes" << endl;
	G.Use_n_Pages (1);
	
	cout << "Started creating the threads" << endl;
	SF_ps.Run (dbf_ps, _ps, cnf_ps, lit_ps); // 161 recs qualified
	J.Run (_s, _ps, _s_ps, cnf_p_ps, lit_p_ps);
	G.Run (_s_ps, _out, grp_order, func);

	cout << "Waiting for everything to be completed" << endl;
	SF_s.WaitUntilDone();
	SF_ps.WaitUntilDone ();
	J.WaitUntilDone ();
	G.WaitUntilDone ();
